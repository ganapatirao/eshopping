      ...
      <div className="container mx-auto px-4">
        <div className="flex justify-between items-center py-4">
          ...
          <Link to="/" className="flex items-center space-x-2">
            {headerData.logoUrl ? (
              <img src={headerData.logoUrl} alt="Logo" className="h-9 w-9 sm:h-10 sm:w-10" />
            ) : (
              <span className="flex items-center justify-center h-9 w-9 sm:h-10 sm:w-10 rounded-xl bg-gradient-to-br from-blue-600 to-purple-600 text-white font-bold shadow-md">
                {(headerData.title || 'S').charAt(0)}
              </span>
            )}
            <h1 className="text-xl sm:text-2xl font-extrabold bg-gradient-to-r from-blue-600 to-purple-600 bg-clip-text text-transparent">{headerData.title}</h1>
          </Link>
                          ...
                          .map(sub => {
                            ...
                            return (
                              <Link
                                key={sub.id}
                                to={sub.link || '#'}
                                className="block px-4 py-2 text-gray-700 hover:bg-gray-100 hover:text-blue-600 transition-colors flex items-center gap-2"
                              >
                                {isSubImageIcon ? (
                                  <img 
                                    src={sub.icon} 
            ...
            {headerData.showUserIcon && (
              user ? (
                <div className="flex items-center gap-1 sm:gap-2">
                  {isAdmin && (
                    <Link to="/admin" title="Admin" className="p-2 text-gray-700 hover:text-purple-600 transition-colors">
                      <ShieldCheck size={22} />
                    </Link>
                  )}
                  <Link to="/orders" title="My Orders" className="p-2 text-gray-700 hover:text-blue-600 transition-colors">
                    <Package size={22} />
                  </Link>
                ...
                <Link to="/login" title="Login" className="p-2 text-gray-700 hover:text-blue-600 transition-colors">
                  <User size={24} />
                </Link>
              )
            )}
                          ...
                          .map(sub => {
                            ...
                            return (
                              <Link
                                key={sub.id}
                                to={sub.link || '#'}
                                className="flex items-center gap-2 py-1 text-gray-600 hover:text-blue-600 transition-colors"
                                onClick={() => setMobileMenuOpen(false)}
                              >
                                {isSubImageIcon ? (
                                  <img 
